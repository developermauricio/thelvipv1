...
<div id="header-wrapper" class="<?php echo esc_attr($header_class); ?>">
    ...
    <?php
        if( !empty( $global_breadcrumb ) ) {
            ...
            triss_breadcrumb_output ( the_title( '<h3>', '</h3>',false ), $breadcrumbs, $bstyle, $style );
        }
    ?><!-- ** Breadcrumb End ** -->
                        ...
                        <div class="column dt-sc-one-half first">
                            <?php if(has_post_thumbnail()) { ?>
                                <div class="entry-thumb">
                                    <a href="<?php the_permalink();?>" title="<?php printf( esc_attr__('%s', 'triss'), the_title_attribute('echo=0') );?>"><?php
                                        $attachment_id  = get_post_thumbnail_id( $post->ID );
                                        $img_attributes = wp_get_attachment_image_src( $attachment_id, 'triss-1170x767' );
