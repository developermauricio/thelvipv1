<?php
	$global_post_styles = array( 'breadcrumb-fixed', 'breadcrumb-parallax', 'overlay', 'overlap', 'boxed', 'custom' );
	...
	$post_style = isset( $post_meta['single-post-style'] ) && in_array( $post_meta['single-post-style'], $global_post_styles ) ? $post_meta['single-post-style'] : 'boxed';
	...
	if( !empty( $post_style ) && $post_style != 'custom' ): ?>
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
            $template = apply_filters( 'triss_blog_single_template', 'framework/templates/single/post-'.$post_style.'.php' );
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
			...
			edit_post_link( esc_html__( ' Edit ','triss' ) ); ?>
        </article><?php
	endif;