...
<?php
	if( $post_Style == 'dt-sc-boxed-style' ): ?>
		<span><?php esc_html_e('By', 'triss'); ?></span>
        <?php echo get_avatar( get_the_author_meta( 'ID' ), 25 ); ?>
		<a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" title="<?php esc_attr_e('View all posts by ', 'triss'); echo get_the_author();?>">
			<?php echo get_the_author();?>
        </a><?php
	elseif( $post_Style == 'dt-sc-overlay-style' ): 
	    $author_data = new WP_User(get_the_author_meta( 'ID' ));
    	$author_role = array_shift($author_data->roles);
		?>
        <i class="fa fa-user"> </i>
        <a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" 
            title="<?php esc_attr_e('View all posts by ', 'triss'); echo get_the_author();?>"><?php echo get_the_author();?></a><?php
