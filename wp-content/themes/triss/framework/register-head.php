 ...
 * --------------------------------------------------------------------------- */
add_action('wp_enqueue_scripts', 'triss_enqueue_scripts');
function triss_enqueue_scripts() {
    ...
    wp_enqueue_script( 'jquery-resizesensor', get_theme_file_uri('/framework/js/ResizeSensor.min.js'), array(), false, true);
    ...
    if( cs_get_option('enable-cookie-consent') == "true" ) {
        wp_enqueue_script( 'triss-cookie-js', get_theme_file_uri('/framework/js/cookieconsent.js'), array(), false, true);
    }

    wp_enqueue_script( 'magnific-popup-js', get_theme_file_uri('/framework/js/magnific/jquery.magnific-popup.min.js'), array(), false, true);
    ...
    if( function_exists( 'is_woocommerce' ) ){
        wp_enqueue_script( 'triss-woocommerce', get_theme_file_uri('/framework/js/woocommerce.js'), array(), false, true);
    }
    wp_enqueue_script( 'triss-jqcustom', get_theme_file_uri('/framework/js/custom.js'), array(), false, true);
    ...
    wp_localize_script( 'triss-jqcustom', 'dttheme_urls', array(
        'theme_base_url'        => esc_js(TRISS_THEME_URI),
        'framework_base_url'    => esc_js(TRISS_THEME_URI).'/framework/',
        'ajaxurl'               => esc_url( admin_url('admin-ajax.php') ),
        'url'                   => esc_url( get_site_url() ),
        'isRTL'                 => esc_js($rtl),
        'loadingbar'            => esc_js($loadingbar),
        'advOptions'            => esc_html__('Show Advanced Options', 'triss'),
    ...
    if( isset($picker) ) {
        ...
        wp_enqueue_script( 'triss-jqcpanel', get_theme_file_uri('/framework/js/controlpanel.js'),array(),false,true);
    }
}
...
* --------------------------------------------------------------------------- */
function triss_scripts_custom() {
    
    $enable_custom_js = (int) get_theme_mod( 'enable-custom-js', triss_defaults('enable-custom-js') );
    $custom_js = get_theme_mod( 'custom-js', '');
    
    if( !empty( $enable_custom_js ) && !empty( $custom_js ) ){
        wp_add_inline_script( 'triss-jqcustom', triss_wp_kses(stripslashes($custom_js)) ,'after');
    }
}
add_action('wp_enqueue_scripts', 'triss_scripts_custom', 100);

add_action( 'admin_enqueue_scripts', 'triss_jq_migration', 0 );
add_action( 'wp_enqueue_scripts', 'triss_jq_migration', 150 );
function triss_jq_migration() {
 ...
 * --------------------------------------------------------------------------- */
add_action( 'wp_enqueue_scripts', 'triss_enqueue_styles', 101 );
function triss_enqueue_styles() {

    // site icons ---------------------------------------------------------------
    if ( ! has_site_icon() ):
        $url = TRISS_THEME_URI . "/images/favicon.ico";
        echo "<link href='$url' rel='shortcut icon' type='image/x-icon' />\n";        
    endif;

    // wp_enqueue_style ---------------------------------------------------------------
    wp_enqueue_style( 'triss', get_stylesheet_uri(), false, TRISS_THEME_VERSION, 'all' );

    wp_enqueue_style( 'triss-base',            get_theme_file_uri('/css/base.css'),         false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-grid',            get_theme_file_uri('/css/grid.css'),         false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-widget',        get_theme_file_uri('/css/widget.css'),         false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-layout',        get_theme_file_uri('/css/layout.css'),         false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-blog',            get_theme_file_uri('/css/blog.css'),         false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-contact',        get_theme_file_uri('/css/contact.css'),        false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-custom-class',    get_theme_file_uri('/css/custom-class.css'),false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'triss-browsers',        get_theme_file_uri('/css/browsers.css'),     false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'prettyphoto',        get_theme_file_uri('/css/prettyPhoto.css'), false, TRISS_THEME_VERSION, 'all' );
    ...
    if( cs_get_option( 'enable-comingsoon' ) )
        wp_enqueue_style( 'triss-comingsoon',  get_theme_file_uri('/css/comingsoon.css'), false, TRISS_THEME_VERSION, 'all' );

    // notfound css
    if ( is_404() )
        wp_enqueue_style( 'triss-notfound',    get_theme_file_uri('/css/notfound.css'), false, TRISS_THEME_VERSION, 'all' );

    // loader css
    $loadingbar = cs_get_option( 'use-site-loader' );
    if( !empty( $loadingbar ) )
        wp_enqueue_style( 'triss-loader',         get_theme_file_uri('/css/loaders.css'), false, TRISS_THEME_VERSION, 'all' );

    // woocommerce css
    if( function_exists( 'is_woocommerce' ) ):
        wp_enqueue_style( 'triss-woo',            get_theme_file_uri('/css/woocommerce.css'), 'woocommerce-general-css', TRISS_THEME_VERSION, 'all' );
        wp_enqueue_style( 'triss-woo-default',    get_theme_file_uri('/css/woocommerce/woocommerce-default.css'), false, TRISS_THEME_VERSION, 'all' );
        wp_enqueue_style( 'triss-woo-hovers',    get_theme_file_uri('/css/woocommerce/woocommerce-hovers.css'), false, TRISS_THEME_VERSION, 'all' );
        wp_enqueue_style( 'triss-woo-custom',    get_theme_file_uri('/css/woocommerce/woocommerce-custom.css'), false, TRISS_THEME_VERSION, 'all' );
    endif;



    // tribe-events -------------------------------------------------------------------
    wp_enqueue_style( 'triss-customevent',         get_theme_file_uri('/tribe-events/custom.css'), false, TRISS_THEME_VERSION, 'all' );
    
    // cookie-consent -----------------------------------------------------------------
    if( cs_get_option('enable-cookie-consent') == "true" ) {
        wp_enqueue_style( 'triss-cookie-css',get_theme_file_uri('/css/cookieconsent.css'), false, TRISS_THEME_VERSION, 'all' );
    }

    wp_enqueue_style( 'triss-popup-css',get_theme_file_uri('/framework/js/magnific/magnific-popup.css'), false, TRISS_THEME_VERSION, 'all' );
    wp_enqueue_style( 'bxslider-css', get_theme_file_uri('/css/jquery.bxslider.min.css'), false, TRISS_THEME_VERSION, 'all' );

    // blog single css ---------------------------------------------------------------------
    wp_enqueue_style( 'triss-custom', get_theme_file_uri('/css/blog-single.css'), false, TRISS_THEME_VERSION, 'all' );
    ...
    // rtl ----------------------------------------------------------------------------
    if(is_rtl()) wp_enqueue_style( 'triss-rtl', get_theme_file_uri('/css/rtl.css'), false, TRISS_THEME_VERSION, 'all' );

    // gutenberg css ---------------------------------------------------------------------
    wp_enqueue_style( 'triss-gutenberg', get_theme_file_uri('/css/gutenberg.css'), false, TRISS_THEME_VERSION, 'all' );
    ...
    if( !empty( $primary_color ) ) {
        ...
        $rgba = triss_hex2rgb( $primary_color );
        $rgba = implode(',', $rgba);
    ...
    if( !empty( $secondary_color ) ) {
        ...
        $rgba = triss_hex2rgb( $secondary_color );
        $rgba = implode(',', $rgba);
    ...
    if( !empty( $tertiary_color ) ) {
        ...
        $rgba = triss_hex2rgb( $tertiary_color );
        $rgba = implode(',', $rgba);
    ...
    if( !empty($primary_color) && !empty($secondary_color) && !empty($tertiary_color) ) {
        ...
        // For Gradient Colors
        $rgba_primary = triss_hex2rgb( $primary_color );
        $rgba_primary = implode(',', $rgba_primary);
        
        $rgba_second = triss_hex2rgb( $secondary_color );
        $rgba_second = implode(',', $rgba_second);
        
        $rgba_third = triss_hex2rgb( $tertiary_color );
    ...
    wp_add_inline_style( 'triss-custom', $css );
    

    $fonts = cs_get_option('custom_font_fields');
    if( isset( $fonts ) && count( $fonts ) > 0 ){
        wp_add_inline_style('triss-custom', triss_styles_custom_font() );
    }
}

add_action( 'wp_enqueue_scripts', 'triss_enqueue_custom_inline', 999 );
if ( ! function_exists( 'triss_enqueue_custom_inline' ) ) {
    function triss_enqueue_custom_inline() {
        wp_register_style( 'triss-custom-inline', '', array(), TRISS_THEME_VERSION, 'all' );
    }
}
 ...
 * --------------------------------------------------------------------------- */
function triss_styles_custom_font() {
    $out = '';
 ...
 * --------------------------------------------------------------------------- */
function triss_body_classes( $classes ) {
    
    // layout
    $classes[]         =     'layout-'. get_theme_mod( 'site-layout', triss_defaults('site-layout') );
    ...
    if( is_page() ) {
        ...
        $fullwidth_blog_archives = cs_get_option( 'enable-fullwidth-blog-archives' );
        $fullwidth_blog_archives = ( isset($fullwidth_blog_archives ) && !empty( $fullwidth_blog_archives ) ) ? true : false;
        if( $fullwidth_blog_archives === true) {
            $classes[] = "fullwidth-page";
        }
        ...
        $fullwidth_single_posts = cs_get_option( 'enable-fullwidth-single-post' );
        $fullwidth_single_posts = ( isset($fullwidth_single_posts ) && !empty( $fullwidth_single_posts ) ) ? true : false;
        if( $fullwidth_single_posts === true) {
            $classes[] = "fullwidth-page";
        }
    ...
    // Enable Fullwidth
    $enable_fullwidth = cs_get_option( 'dt-woo-enable-fullwidth' );
    $enable_fullwidth = ( isset($enable_fullwidth ) && !empty( $enable_fullwidth ) ) ? true : false;
    if($enable_fullwidth) {
        if(function_exists( 'is_woocommerce' ) && (is_shop() || is_singular('product') || is_product_category() || is_product_tag() ) ) {
            $classes[] = 'fullwidth-page';
        }
    }
...
add_filter( 'body_class', 'triss_body_classes' );?>