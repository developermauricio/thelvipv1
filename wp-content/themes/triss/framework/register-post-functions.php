...
class triss_post_functions {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_triss_post_rating_like', array( $this, 'triss_post_rating_like' ) );
		add_action( 'wp_ajax_nopriv_triss_post_rating_like', array( $this, 'triss_post_rating_like' ) );

		add_filter( 'post_class', array( $this, 'triss_add_remove_post_class' ), 10, 1 );

		add_action( 'wp_ajax_triss_ajax_load_more_post', array( $this, 'triss_ajax_load_more_post' ) );
		add_action( 'wp_ajax_nopriv_triss_ajax_load_more_post', array( $this, 'triss_ajax_load_more_post' ) );
	}

	function triss_post_meta_fields( $ajax = false, $page_ID = 0 ) {
	...
	static function triss_post_view_count($pid = '') {
	...
	static function triss_post_like_count($pid = '') {
	...
	function triss_post_rating_like() {
		...
		if ( wp_verify_nonce( $nonce, 'rating-nonce' ) && $postid > 0 ) {
			...
			$out = esc_html__('Security check', 'triss');
		}
	...
	function triss_add_remove_post_class( $classes ) {
	...
	function triss_ajax_load_more_post() {
		...
		if($the_query->have_posts()):
			...
			if( empty($meta) ) {
				$obj = new triss_post_functions;
				$meta = $obj->triss_post_meta_fields(true);
			}
            ...
            <h2><?php esc_html_e('Nothing Found.', 'triss'); ?></h2>
            <p><?php esc_html_e('Apologies, but no results were found for the requested archive.', 'triss'); ?></p><?php
        endif;
