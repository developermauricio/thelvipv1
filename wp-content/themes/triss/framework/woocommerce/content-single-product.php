...
if( ! function_exists( 'triss_woo_product_single_template_option' ) ) {

	function triss_woo_product_single_template_option() {
...
if( ! function_exists( 'triss_woo_product_single_template' ) ) {

	function triss_woo_product_single_template( $single_template ) {
		...
		if (is_singular( 'product' )) {
			...
			if( $product_template == 'custom-template' ) {
				$single_template = TRISS_THEME_DIR. '/framework/woocommerce/templates/custom-template.php';
			}
	...
	add_filter('template_include', 'triss_woo_product_single_template', 100);

}

/** Product additional tabs **/

if( ! function_exists( 'triss_woo_additional_product_tabs' ) ) {

	function triss_woo_additional_product_tabs( $tabs ) {
		...
		if( is_array( $product_additional_tabs ) && !empty( $product_additional_tabs ) ) {
			...
			foreach( $product_additional_tabs as $product_additional_tab_key => $product_additional_tab ) {
				...
				$tabs[$tab_key] = array(
					'title' 	=> $tab_title,
					'priority' 	=> $priority,
					'callback' 	=> 'triss_woo_additional_product_tabs_content'
				);
	...
	function triss_woo_additional_product_tabs_content( $key ) {
		...
		if( is_array( $product_additional_tabs ) && !empty( $product_additional_tabs ) ) {
			...
			foreach( $product_additional_tabs as $product_additional_tab_key => $product_additional_tab ) {
				...
				if($tab_key == $key) {
					...
					echo triss_html_output(do_shortcode($tab_description));
	...
	add_filter( 'woocommerce_product_tabs', 'triss_woo_additional_product_tabs', 10 );
...
if ( ! function_exists( 'triss_woo_shop_single_addtocart_sticky' ) ) {

	function triss_woo_shop_single_addtocart_sticky() {
			...
			echo '<div class="container">';
				echo '<div class="dt-sc-shop-single-sticky-addtocart-content">';
					...
					echo '<div class="dt-sc-shop-single-sticky-addtocart-info">';
						echo '<h3>'.triss_html_output($product->get_name()).'</h3>';
						echo wc_get_rating_html( $product->get_average_rating() );
					echo '</div>';				
				echo '</div>';
				echo '<div class="dt-sc-shop-single-sticky-addtocart-section">';
					echo '<span class="dt-sc-shop-single-sticky-addtocart-price">'.triss_html_output($product->get_price_html()).'</span>';	
					if(triss_check_item_is_in_cart( $product->get_id() )) {
						echo '<span class="dt-sc-shop-single-sticky-addtocart-added">'.esc_html__('Product Added To Cart', 'triss').'</span>';	
					} else {
	...
	add_action( 'wp_footer', 'triss_woo_shop_single_addtocart_sticky', 11 );
...
if( ! function_exists( 'triss_woo_show_upsell' ) ) {
	
	function triss_woo_show_upsell() {

		global $product;

		$product_id = $product->get_id();
		$settings   = triss_single_product_settings( $product_id );
		...
		if( isset( $settings['show-upsell'] ) && ( $settings['show-upsell'] == 'true' || $settings['show-upsell'] === true ) ) {
			...
			$display_mode = triss_woo_post_display_mode_from_location($product_style_template);
			if($display_mode == 'list') {
				$settings['upsell-column'] = 1;	
			}

			triss_product_style_setup_template_prop($product_style_template); /* Call Product Style Variables Setup */

			woocommerce_upsell_display( $limit = $settings['upsell-limit'], $columns = $settings['upsell-column'], $orderby = 'rand', $order = 'desc' );

			triss_product_style_reset_template_prop();  /* Reset Product Style Variables Setup */

		}

	}

	add_action( 'woocommerce_after_single_product_summary', 'triss_woo_show_upsell', 15 );
...
if( ! function_exists( 'triss_woo_show_related_products' ) ) {

	function triss_woo_show_related_products() {
	...
	add_action( 'woocommerce_after_single_product_summary', 'triss_woo_show_related_products', 20 );
...
if($enable_size_guide) {
	add_action( 'woocommerce_single_product_summary', 'triss_woo_loop_product_button_elements_sizeguide', 35 );
}
...
if( ! function_exists( 'triss_woo_loop_product_additional_labels' ) ) {

	function triss_woo_loop_product_additional_labels( $single_template ) {
		...
		triss_woo_show_product_additional_labels();

	}

	add_action('dt_woo_loop_product_additional_labels', 'triss_woo_loop_product_additional_labels');

}

/* Product Labels - Shortcodes */

if( ! function_exists( 'triss_woo_show_product_additional_labels' ) ) {

	function triss_woo_show_product_additional_labels() {
		...
		if( $product->is_on_sale() && $product->is_in_stock() ) {
			echo '<span class="onsale"><span>'.esc_html__('Sale', 'triss').'</span></span>';
		} else if( !$product->is_in_stock() ) {
			echo '<span class="out-of-stock"><span>'.esc_html__('Sold Out','triss').'</span></span>';
		}
		...
		if( $product->is_featured() ) {
			echo '<div class="featured-tag">
						<div>
							...
							<span>'.esc_html__('Featured', 'triss').'</span>
						</div>
					</div>';
		}

		if(isset($settings['product-new-label']) && $settings['product-new-label'] == 'true') {
			echo '<span class="new"><span>'.esc_html__('New', 'triss').'</span></span>';
		}	
