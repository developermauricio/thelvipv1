...
if ( ! function_exists( 'dt_sc_woo_default_product_settings' ) ) {
	...
	function dt_sc_woo_default_product_settings() {
		...
		$settings["template-title"]= esc_html__("Default Template", 'triss');
		$settings["product-style"]="product-style-default";
		$settings["product-hover-styles"]="";
		$settings["product-overlay-bgcolor"]="rgba(255,255,255,0.7)";
		$settings["product-overlay-dark-bgcolor"]="1";
		$settings["product-overlay-effects"]="product-overlay-righttoleft";
		$settings["product-hover-image-effects"]="product-hover-image-fadeinleft";
		$settings["product-hover-secondary-image-effects"]="product-hover-secimage-slideright";
		$settings["product-content-hover-effects"]="product-content-hover-slidebottom";
		...
		$settings["product-bordershadow-highlight"]="product-bordershadow-highlight-default";
		...
		$settings["product-thumb-alignment"]="product-thumb-alignment-bottom-right";
		$settings["product-thumb-iconsgroup-icons"]=array ( 'cart','compare', 'wishlist', 'quickview' );
		$settings["product-thumb-iconsgroup-style"]="product-thumb-iconsgroup-style-bgfill-square";
		$settings["product-thumb-iconsgroup-position"]="product-thumb-iconsgroup-position-vertical vertical-position-bottom-right";
		...
		$settings["product-content-iconsgroup-icons"]=array ( 'cart' );
		$settings["product-content-iconsgroup-style"]="product-content-iconsgroup-style-bgfill-rounded";
		$settings["product-content-buttonelement-button"]="";
		$settings["product-content-buttonelement-secondary-button"]="";
		$settings["product-content-buttonelement-style"]="product-content-buttonelement-style-bgfill-rounded";
...
if ( ! function_exists( 'dt_sc_woo_shop_products_class' ) ) {
	...
	function dt_sc_woo_shop_products_class() {
		...
		if(isset($woo_product_style_template) && (int)$woo_product_style_template > 0) {
			...
			$woo_product_style_templates = cs_get_option( 'dt-woo-product-style-templates' );
			$product_style_templates = (isset($woo_product_style_templates[0]) && !empty($woo_product_style_templates[0])) ? $woo_product_style_templates[0] : dt_sc_woo_default_product_settings();
...
if ( ! function_exists( 'triss_product_style_setup_template_prop' ) ) {

	function triss_product_style_setup_template_prop($woo_product_style_template) {
...
if ( ! function_exists( 'triss_product_style_reset_template_prop' ) ) {

	function triss_product_style_reset_template_prop() {
...
if ( ! function_exists( 'triss_woo_post_display_mode_from_location' ) ) {

	function triss_woo_post_display_mode_from_location($woo_product_style_template) {
...
if ( ! function_exists( 'triss_woo_post_class' ) ) {

	function triss_woo_post_class($classes, $class = '', $post_id = '') {
	 ...
	 add_filter('post_class', 'triss_woo_post_class', 21, 3 );

}

/** Updating Product Category Loop Class **/

if ( ! function_exists( 'triss_woo_product_cat_class' ) ) {

	function triss_woo_product_cat_class($classes, $class = '', $category) {
	...
	add_filter('product_cat_class', 'triss_woo_product_cat_class', 21, 3 );
...
if ( ! function_exists( 'triss_woo_loop_product_button_elements_cart' ) ) {

	function triss_woo_loop_product_button_elements_cart() {
		...
		if( !empty($add_to_cart) ) {
			...
			echo '<div class="wcct_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Add To Cart', 'triss' ).'">'.triss_html_output($add_to_cart).'</div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_cart', 'triss_woo_loop_product_button_elements_cart' );

}

// Wishlist

if ( ! function_exists( 'triss_woo_loop_product_button_elements_wishlist' ) ) {

	function triss_woo_loop_product_button_elements_wishlist() {
		...
		if ( shortcode_exists( 'yith_wcwl_add_to_wishlist' ) ) {
			...
			echo '<div class="wcwl_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Wishlist', 'triss' ).'">'.do_shortcode('[yith_wcwl_add_to_wishlist product_id="' . $product_id . '"]').'</div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_wishlist', 'triss_woo_loop_product_button_elements_wishlist' );

}

// Quick View

if ( ! function_exists( 'triss_woo_loop_product_button_elements_quickview' ) ) {

	function triss_woo_loop_product_button_elements_quickview() {
		...
		if ( shortcode_exists( 'yith_quick_view' ) ) {
			...
			echo '<div class="wcqv_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Quick View', 'triss' ).'">'.do_shortcode('[yith_quick_view product_id="' . $product_id . '"]').'</div>';
		
		}

	}

	add_action( 'dt_woo_loop_product_button_elements_quickview', 'triss_woo_loop_product_button_elements_quickview' );

}

// Compare

if ( ! function_exists( 'triss_woo_loop_product_button_elements_compare' ) ) {

	function triss_woo_loop_product_button_elements_compare() {
		...
		if( class_exists( 'YITH_Woocompare' ) ) {
			...
			$button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Compare', 'triss' ) );
			...
			echo '<div class="wccm_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Compare', 'triss' ).'"><a href="'.esc_url_raw( add_query_arg( $url ) ).'" class="'.esc_attr($class).'" data-product_id="'.esc_attr($product->get_id()).'" rel="nofollow">'.triss_html_output($button_text).'</a></div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_compare', 'triss_woo_loop_product_button_elements_compare' );

}

// Swatches

if ( ! function_exists( 'triss_woo_loop_product_button_elements_swatches' ) ) {

	function triss_woo_loop_product_button_elements_swatches() {

		// Swatches
		triss_woo_loop_product_content_swatches();

	}

	add_action( 'dt_woo_loop_product_button_elements_swatches', 'triss_woo_loop_product_button_elements_swatches' );

}

// Size Guide

if ( ! function_exists( 'triss_woo_loop_product_button_elements_sizeguide' ) ) {

	function triss_woo_loop_product_button_elements_sizeguide() {
		...
		if($single_product_size_guides) {
			...
			echo '<div class="wcsg_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Size Guide', 'triss' ).'"><a href="#" class="button dt-wcsg-button" data-product_id="'.esc_attr($product->get_id()).'">'.esc_html__('Size Guide', 'triss' ).'</a></div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_sizeguide', 'triss_woo_loop_product_button_elements_sizeguide' );
