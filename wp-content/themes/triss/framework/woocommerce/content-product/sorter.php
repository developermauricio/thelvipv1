...
if( ! function_exists( 'triss_product_style_setup_template_prop_action' ) ) {
	
	function triss_product_style_setup_template_prop_action() {
		...
		triss_product_style_setup_template_prop($product_style_template);

	}

	add_action( 'woocommerce_before_shop_loop', 'triss_product_style_setup_template_prop_action', 5 );
...
if($show_sorter_on_header) {
	...
	if(!empty($sorter_header_elements)) {
		...
		add_action( 'woocommerce_before_shop_loop', 'triss_woo_sorting_wrapper', 9 );
		...
		foreach($sorter_header_elements as $sorter_header_element_key => $sorter_header_element) {
			...
			if($sorter_header_element_key == 'filter') {
				...
				$sorter_function_name = 'triss_woo_display_mode_options';
			} else if($sorter_header_element_key == 'display_mode') {
				$sorter_function_name = 'triss_woo_display_mode';				
			}

			$cnt = 0;
			if($total_items > 2 && $j == 2) {
				add_action('woocommerce_before_shop_loop', 'triss_woo_sorter_center_item_start_div', $i);
				$i = $i+1;
			}

			add_action('woocommerce_before_shop_loop', 'triss_woo_sorter_item_start_div', ($i));
			add_action('woocommerce_before_shop_loop', $sorter_function_name, ($i+1));
			add_action('woocommerce_before_shop_loop', 'triss_woo_sorter_item_end_div', ($i+2));

			if($total_items > 2 && $j == ($total_items-1)) {
				add_action('woocommerce_before_shop_loop', 'triss_woo_sorter_center_item_end_div', ($i+3));
				$i = $i+1;
			}			
		...
		add_action( 'woocommerce_before_shop_loop', 'triss_woo_sorting_wrapper_close', 51 );
...
if($show_sorter_on_footer) {
	...
	if(!empty($sorter_footer_elements)) {
		...
		add_action( 'woocommerce_after_shop_loop', 'triss_woo_sorting_wrapper', 9 );
		...
		foreach($sorter_footer_elements as $sorter_footer_element_key => $sorter_footer_element) {
			...
			$cnt = 0;
			if($total_items > 2 && $j == 2) {
				add_action('woocommerce_after_shop_loop', 'triss_woo_sorter_center_item_start_div', $i);
				$i = $i+1;
			}
			
			add_action('woocommerce_after_shop_loop', 'triss_woo_sorter_item_start_div', $i);
			add_action('woocommerce_after_shop_loop', $sorter_function_name, ($i+1));
			add_action('woocommerce_after_shop_loop', 'triss_woo_sorter_item_end_div', ($i+2));

			if($total_items > 2 && $j == ($total_items-1)) {
				add_action('woocommerce_after_shop_loop', 'triss_woo_sorter_center_item_end_div', ($i+3));
				$i = $i+1;
			}			
		...
		add_action( 'woocommerce_after_shop_loop', 'triss_woo_sorting_wrapper_close', 51 );
...
if ( ! function_exists( 'triss_woo_sorting_wrapper' ) ) {

	function triss_woo_sorting_wrapper() {
		echo '<div class="product-loop-sorting">';
	}

}

if ( ! function_exists( 'triss_woo_sorting_wrapper_close' ) ) {

	function triss_woo_sorting_wrapper_close() {
		echo '</div>';
	}

}

if ( ! function_exists( 'triss_woo_sorter_item_start_div' ) ) {

	function triss_woo_sorter_item_start_div() {
		echo '<div class="product-loop-sorting-item">';
	}

}

if ( ! function_exists( 'triss_woo_sorter_item_end_div' ) ) {

	function triss_woo_sorter_item_end_div() {
		echo '</div>';
	}

}

if ( ! function_exists( 'triss_woo_sorter_center_item_start_div' ) ) {

	function triss_woo_sorter_center_item_start_div() {
		echo '<div class="product-loop-sorting-item-group">';
	}

}

if ( ! function_exists( 'triss_woo_sorter_center_item_end_div' ) ) {

	function triss_woo_sorter_center_item_end_div() {
		echo '</div>';
	}

}

if( ! function_exists( 'triss_woo_pagination' ) ) {

	function triss_woo_pagination() {
		echo '<div class="pagination">';
			echo triss_pagination();
		echo '</div>';
	}

}

if( ! function_exists( 'triss_woo_display_mode_options' ) ) {

	function triss_woo_display_mode_options() {
		...
		if( is_shop() || is_product_category() || is_product_tag() ) {
			...
			echo '<div class="product-layout-controller '.esc_attr($grid_controller_class).'">';
				echo '<ul class="product-change-layout">';
					echo '<li class="hidden"><span data-column="1" class="'.esc_attr($one_column_class).'">'.esc_html__('1', 'triss').'</span></li>';
					echo '<li><span data-column="2" class="'.esc_attr($two_column_class).'">'.esc_html__('2', 'triss').'</span></li>';
					echo '<li><span data-column="3" class="'.esc_attr($three_column_class).'">'.esc_html__('3', 'triss').'</span></li>';
				echo '</ul>';
			echo '</div>';	
			...
			echo '<div class="product-list-options-controller '.esc_attr($list_controller_class).'">';
				echo '<ul class="product-list-options">';
					echo '<li><span data-list-option="left-thumb" class="'.esc_attr($left_thumb_class).'">'.esc_html__('Left Thumb', 'triss').'</span></li>';
					echo '<li><span data-list-option="right-thumb" class="'.esc_attr($right_thumb_class).'">'.esc_html__('Right Thumb', 'triss').'</span></li>';
				echo '</ul>';
			echo '</div>';
...
if( ! function_exists( 'triss_woo_display_mode' ) ) {

	function triss_woo_display_mode() {
		...
		if( is_shop() || is_product_category() || is_product_tag() ) {
			...
			echo '<div class="product-display-controller">';
				echo '<ul class="product-change-display">';
					echo '<li><span data-display="grid" class="'.esc_attr($grid_display_mode_class).'">'.esc_html__('Grid', 'triss').'</span></li>';
					echo '<li><span data-display="list" class="'.esc_attr($list_display_mode_class).'">'.esc_html__('List', 'triss').'</span></li>';
				echo '</ul>';
			echo '</div>';	
