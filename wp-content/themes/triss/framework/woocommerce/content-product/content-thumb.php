...
if( ! function_exists( 'triss_woo_loop_product_thumbnail' ) ) {
	
	function triss_woo_loop_product_thumbnail() {
		...
		echo '<div class="product-thumb">';
			...
			echo '<a class="image" href="'.esc_url($product->get_permalink()).'" title="'.esc_attr($product->get_name()).'">';
				...
				if($product_hover_styles != 'product-hover-egrpovrcnt') {
					echo '<div class="product-thumb-overlay" '.triss_html_output($product_overlay_bgcolor).'></div>';
				}
			...
			if($product_thumb_content) {
				...
				triss_woo_loop_product_thumb_content_setup($product_thumb_content);

				echo '<div class="product-thumb-content" '.triss_html_output($product_thumb_content_overlay_bgcolor).'>';
	...
	add_action( 'woocommerce_before_shop_loop_item_title', 'triss_woo_loop_product_thumbnail', 10 );
