...
if ( ! function_exists( 'triss_html_output' ) ) {

	function triss_html_output( $html ) {

		return apply_filters( 'triss_html_output', $html );
...
if( ! function_exists( 'triss_shop_archive_page' ) ) {

	function triss_shop_archive_page() {
...
if( ! function_exists( 'triss_single_product' ) ) {

	function triss_single_product( $id , $position ) {
...
if( ! function_exists( 'triss_single_product_settings' ) ) {

	function triss_single_product_settings ( $id ) {
...
if( ! function_exists( 'triss_woo_remove_anonymous_object_action' ) ) {

	function triss_woo_remove_anonymous_object_action( $tag, $class, $method, $priority = null ){
...
if( ! function_exists( 'triss_woo_loop_column_class' ) ) {

	function triss_woo_loop_column_class($columns) {
...
if(!function_exists('triss_check_item_is_in_cart')) {
	function triss_check_item_is_in_cart( $product_id ){
...
if( ! function_exists( 'triss_single_product_nav' ) ) {
	function triss_single_product_nav() {
		...
		$output .= '<div class="dt-sc-single-product-nav">';
			...
			if ( ! empty( $prev ) ) {
				...
				$output .= '<div class="dt-sc-single-product-nav-btn product-nav-prev">';
					$output .= '<a href="'.esc_url( $prev->get_permalink() ).'">'.esc_html__('Previous product', 'triss').'<span class="product-nav-btn-icon"></span></a>';
			...
			$output .= '<a href="'.esc_url($shop_page_url).'" class="dt-sc-single-product-nav-back-btn"><span>'.esc_html__('Back to products', 'triss').'</span></a>';

			if ( ! empty( $next ) ) {

				$output .= '<div class="dt-sc-single-product-nav-btn product-nav-next">';
					$output .= '<a href="'.esc_url( $next->get_permalink() ).'">'.esc_html__('Next product', 'triss').'<span class="product-nav-btn-icon"></span></a>';
					$output .= '<div class="dt-sc-single-product-nav-intro-wrapper">';
						$output .= '<div class="product-nav-intro">';
							$output .= '<div class="product-nav-intro-image">';
...
if( ! function_exists( 'triss_single_product_sociable_share_follow' ) ) {
	function triss_single_product_sociable_share_follow($product_id, $share_follow_type, $social_icon_style_cls, $social_icon_radius_cls, $social_icon_inline_alignment_cls) {
	    ...
	    if($share_follow_type == 'share') {
			...
			$output .= '<div class="dt-sc-single-product-share-container '.esc_attr($social_icon_style_cls).' '.esc_attr($social_icon_radius_cls).' '.esc_attr($social_icon_inline_alignment_cls).'">';
				...
				$output .= '<a class="dt-sc-single-product-share-item-icon">'.esc_html__('Share', 'triss').'</a>';
			...
			if( is_array( $sociable_flds ) && !empty( $sociable_flds ) ) {
				...
				if(!empty( $list )) {
					...
					$output .= '<div class="dt-sc-single-product-follow-container '.esc_attr($social_icon_style_cls).' '.esc_attr($social_icon_radius_cls).' '.esc_attr($social_icon_inline_alignment_cls).'">';
						$output .= '<a class="dt-sc-single-product-follow-item-icon">'.esc_html__('Follow', 'triss').'</a>';
...
if ( ! function_exists( 'triss_products_ajax_call' ) ) {

	function triss_products_ajax_call() {
	...
	add_action( 'wp_ajax_triss_products_ajax_call', 'triss_products_ajax_call' );
	add_action( 'wp_ajax_nopriv_triss_products_ajax_call', 'triss_products_ajax_call' );

}

// Product Shortcode - Ajax Pagination

if ( ! function_exists( 'triss_products_ajax_pagination' ) ) {

	function triss_products_ajax_pagination($max_num_pages, $current_page, $post_per_page, $function_call, $output_div, $shortcode_attrs) {
...
if ( ! function_exists( 'triss_products_sale_countdown_timer' ) ) {

	function triss_products_sale_countdown_timer() {

		$product_template = triss_woo_product_single_template_option();
	...
	add_action( 'woocommerce_single_product_summary', 'triss_products_sale_countdown_timer', 15 );

}

// Product Shortcode - Ajax Call

if ( ! function_exists( 'triss_size_guide_popup' ) ) {

	function triss_size_guide_popup() {
		...
		if($product_id > -1) {
			...
			if($single_product_size_guides) {
				echo '<div class="dt-sc-size-guide-popup-container">';
					...
					echo '</div>';
					echo '<div class="dt-sc-size-guide-popup-close">'.esc_html__('Close','triss').'</div>';
				echo '</div>';
			}		
	...
	add_action( 'wp_ajax_triss_size_guide_popup', 'triss_size_guide_popup' );
	add_action( 'wp_ajax_nopriv_triss_size_guide_popup', 'triss_size_guide_popup' );

}


// Shop Widgets Filter - Color Attribute

if(!function_exists('triss_woocommerce_layered_nav_term_html')) {
	function triss_woocommerce_layered_nav_term_html($term_html, $term, $link, $count) {

		$color = get_term_meta ( $term->term_id, 'slctd_clr' );

		if(is_array($color) && !empty($color)) {
			$term_html = '<span class="woocommerce-widget-bg-color" style="background-color:'.$color[0].';"></span>'.$term_html;
		} else {
	...
	add_filter('woocommerce_layered_nav_term_html', 'triss_woocommerce_layered_nav_term_html', 10, 4);
}
