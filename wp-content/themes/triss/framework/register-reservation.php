<?php
add_action( 'wp_ajax_triss_fill_staffs', 'triss_fill_staffs' ); # For logged-in users
add_action( 'wp_ajax_nopriv_triss_fill_staffs','triss_fill_staffs'); # For logged-out users 
function triss_fill_staffs() {
	if( isset($_REQUEST['service_id']) ){
		...
		echo "<option value=''>".esc_html__('Select','triss')."</option>";
...
//appointment type2
add_action( 'wp_ajax_triss_dt_generate_schedule', 'triss_dt_generate_schedule' );
add_action( 'wp_ajax_nopriv_triss_dt_generate_schedule','triss_dt_generate_schedule');
function triss_dt_generate_schedule() {
	...
	$out = '';
	$out .= '<h2>'.esc_html__('Select Time','triss').'</h2><div class="dt-sc-single-border-separator"></div><div class="dt-sc-hr-invisible-small"></div>';
	...
	foreach( $staffs_arr as $sid => $sname ) {
		...
		if( count($working_hours) ){
			$slot = triss_dt_findTimeSlot( $working_hours, $bookings, $seldate, $service_duration );
			if( !empty($slot) ){
				$slots[] = $slot;
			}
		}
		...
		if( !empty($slots) ) {
			...
			$out .= '<div class="dt-sc-info-box">'.esc_html__('Time slots not available for your requested date!. Please try again by changing date.', 'triss').'</div>';	
		}
	} # Staffs loops end
	...
	if( $out != '' ) {
		...
		echo '<p class="dt-sc-info-box">'.esc_html__('No Time slots available','triss').'</p>';
	}
...
add_action( 'wp_ajax_triss_available_times', 'triss_available_times' ); # For logged-in users
add_action( 'wp_ajax_nopriv_triss_available_times', 'triss_available_times' ); # For logged-out users
function triss_available_times(){
	...
	foreach( $mgs as $sid => $sname ) {
		...
		if( count($working_hours) ){
			...
			while( $i < $loop ){
				$slot = triss_findTimeSlot( $working_hours, $bookings, $date , $service_duration );
				if( !empty($slot) ){
					$slots[] = $slot;
				}
...
function triss_findTimeSlot( $working_hours, $bookings, $date , $service_duration = 1800 ){
	...
	if( array_key_exists($dayofweek, $working_hours)  && !$past ){
		...
		if( $show ) {
			...
			$intersection = triss_findInterSec( $working_start_time,$working_hours[ $dayofweek ][ 'end' ],$_REQUEST['stime'],$_REQUEST['etime']);

			for( $time = triss_StrToTime($intersection['start']); $time <= ( triss_StrToTime($intersection['end']) - $service_duration ); $time += $service_duration ){
				...
				if( !in_array($value, $bookings) ) { # if already booked in $time
					...
					$object->end = $end;
					$translatable_day =  triss_translatableDay( date('l',strtotime($date)) );
...
//appointment type2
function triss_dt_findTimeSlot( $working_hours, $bookings, $date , $service_duration = 1800 ){
	...
	if( array_key_exists($dayofweek, $working_hours)  && !$past ){
		...
		if( $show ) {
			...
			$intersection = triss_findInterSec( $working_start_time,$working_hours[ $dayofweek ][ 'end' ],'00:00','23:59');
...
function triss_translatableDay( $day ) {
	
	switch( $day ):
		case 'Sunday':
			$day = esc_html__('Sunday','triss');
		break;
	
		case 'Monday':
			$day = esc_html__('Monday','triss');
		break;

		case 'Tuesday':
			$day = esc_html__('Tuesday','triss');
		break;

		case 'Wednesday':
			$day = esc_html__('Wednesday','triss');
		break;

		case 'Thursday':
			$day = esc_html__('Thursday','triss');
		break;

		case 'Friday':
			$day = esc_html__('Friday','triss');
		break;

		case 'Saturday':
			$day = esc_html__('Saturday','triss');
		break;
	endswitch;
...
function triss_findInterSec( $p1_start, $p1_end, $p2_start, $p2_end ) {
	$result = false;
...
//appointment type2
function triss_appointment_type2_customer( $customer ){
...
add_action( 'wp_ajax_triss_new_reservation', 'triss_new_reservation' ); # For logged-in users
add_action( 'wp_ajax_nopriv_triss_new_reservation','triss_new_reservation'); # For logged-out users
function triss_new_reservation(){
	global $wpdb;
	...
	#New Customer
		...
		$customer = triss_customer($name,$email,$phone);
	#New Customer
	
	$id = $wpdb->get_var("SELECT max(option_id) FROM $wpdb->options");
	$title = esc_html__("New Reservation By ",'triss').$name;
	$body =  $_REQUEST['body'];
	...
	$amount = ( ($staff_price+$service_price) > 0 ) ?  triss_currency_symbol( cs_get_option('appointment-currency') ).' '.( $staff_price+$service_price ) : $amount;
	...
	$array = array(
		...
		$subject = triss_replace( $subject, $array);
		
		$message =  cs_get_option('appointment_notification_to_admin_message' );
		$message = triss_replace( $message, $array);
		triss_send_mail( $admin_email, $subject, $message);
		...
		$message = triss_replace( $message, $array);
		triss_send_mail( $sinfo["emailid"], $subject, $message);
		...
		if( !empty($client_email) ) {
			...
			triss_send_mail( $client_email, $subject, $message);
		}
	...
	if( update_option( $option, $data ) ){
		...
		$url = triss_get_page_permalink_by_its_template('tpl-reservation.php');
...
add_action( 'wp_ajax_triss_paypal_request', 'triss_paypal_request' ); # For logged-in users
add_action( 'wp_ajax_nopriv_triss_paypal_request','triss_paypal_request'); # For logged-out users
function triss_paypal_request() {
	...
	if( update_option( $option, $data ) ) {
		...
		$url = add_query_arg( array( 
			'cmd' => '_xclick',
			'item_name' => esc_html__("Service :",'triss').' '.get_the_title($service).' - '. esc_html__("Time :",'triss').$_REQUEST['date'].'('.$_REQUEST['time'].')',
			'item_number' => $option,
			'business' => $uname,
...
add_action( 'wp_loaded', 'triss_paypal_listener' ); # Paypal ExpressCheckout redirect 
function triss_paypal_listener() {
...
/* Calender */
function triss_replace( $content , $array ){
    $replace = array(
...
function triss_replace_agenda( $content , $array ){
    $replace = array(
     '[STAFF_NAME]' => $array['staff_name'],
...
function triss_currency_symbol( $currency = '' ) {
	switch( $currency ) {
		case 'AUD': 
		case 'CAD': 
		case 'HKD':
...
function triss_currencies() {
	return array_unique( array(
		'AUD' => esc_html__('Australian Dollar','triss').' - '.'AUD',
		'BRL' => esc_html__('Brazilian Real ','triss').' - '.'BRL',
		'CAD' => esc_html__('Canadian Dollar','triss').' - '.'CAD',
		'CZK' => esc_html__('Czech Koruna','triss').' - '.'CZK',
		'DKK' => esc_html__('Danish Krone','triss').' - '.'DKK',
		'EUR' => esc_html__('Euro','triss').' - '.'EUR',
		'HKD' => esc_html__('Hong Kong Dollar','triss').' - '.'HKD',
		'HUF' => esc_html__('Hungarian Forint','triss').' - '.'HUF',
		'ILS' => esc_html__('Israeli New Sheqel','triss').' - '.'ILS',
		'JPY' => esc_html__('Japanese Yen','triss').' - '.'JPY',
		'MYR' => esc_html__('Malaysian Ringgit','triss').' - '.'MYR',
		'MXN' => esc_html__('Mexican Peso','triss').' - '.'MXN',
		'NOK' => esc_html__('Norwegian Krone','triss').' - '.'NOK',
		'NZD' => esc_html__('New Zealand Dollar','triss').' - '.'NZD',
		'PHP' => esc_html__('Philippine Peso','triss').' - '.'PHP',
		'PLN' => esc_html__('Polish Zloty','triss').' - '.'PLN',
		'GBP' => esc_html__('Pound Sterling','triss').' - '.'GBP',
		'RUB' => esc_html__('Russian Ruble','triss').' - '.'RUB',
		'SGD' => esc_html__('Singapore Dollar','triss').' - '.'SGD',
		'SEK' => esc_html__('Swedish Krona','triss').' - '.'SEK',
		'CHF' => esc_html__('Swiss Franc','triss').' - '.'CHF',
		'TWD' => esc_html__('Taiwan New Dollar','triss').' - '.'TWD',
		'THB' => esc_html__('Thai Baht','triss').' - '.'THB',
		'TRY' => esc_html__('Turkish Lira','triss').' - '.'TRY',
		'USD' => esc_html__('U.S. Dollar','triss').' - '.'USD'
	) );
}

function triss_currency_with_symbol() {

	$cws = array();
	$currencies = triss_currencies();

	foreach( $currencies as $code => $name ) {
		$symbol = triss_currency_symbol( $code );
...
function triss_durToString( $duration ) {

    $hours   = (int)( $duration / 3600 );
    $minutes = (int)( ( $duration % 3600 ) / 60 );
    $result  = '';
    if ( $hours > 0 ) {
        $result = sprintf( esc_html__( '%d h', 'triss' ), $hours );
        if ( $minutes > 0 ) {
            $result .= ' ';
        }
    }
    ...
    if ( $minutes > 0 ) {
        $result .= sprintf( esc_html__( '%d min', 'triss' ), $minutes );
    }
 return $result;
}

function triss_dates_range( $start_date, $end_date, $days = array() ){
...
function triss_send_mail( $to, $subject, $message ){
...
//appointment type2
add_action( 'wp_ajax_triss_dttheme_new_reservation', 'triss_dttheme_new_reservation' ); # For logged-in users
add_action( 'wp_ajax_nopriv_triss_dttheme_new_reservation','triss_dttheme_new_reservation'); # For logged-out users
function triss_dttheme_new_reservation(){
	global $wpdb;
	...
	$customer = triss_appointment_type2_customer($customer_data);
	#New Customer

	$id = $wpdb->get_var("SELECT max(option_id) FROM $wpdb->options");
	$title = esc_html__("New Reservation By ",'triss').$firstname;
