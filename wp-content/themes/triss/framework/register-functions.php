 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('triss_features')) {

	function triss_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'triss-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'triss-blog-i-column', 900, 550, true  ); 	// blog - i column
			add_image_size( 'triss-blog-ii-column', 750, 500, true  ); 	// blog - ii column
			add_image_size( 'triss-blog-iii-column', 570, 380, true  ); 	// blog - iii column
			add_image_size( 'triss-blog-list', 600, 400, true  ); 	// blog - list
			add_image_size( 'triss-blog-ii-column-masonry', 750 ); 	// blog - ii column masonry
			add_image_size( 'triss-blog-iii-column-masonry', 540 ); 	// blog - iii column masonry

			add_image_size( 'triss-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'triss-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'triss-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'triss-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'triss-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		}
		...
		register_nav_menus( array(
			'main-menu' => esc_html__('Main Menu', 'triss'),
		) );
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'triss' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'triss' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'triss' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			)
		));
	}
	add_action('after_setup_theme', 'triss_features');
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('triss_under_construction') ){
	function triss_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'triss_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'triss_rand_css', 101);
	function triss_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, TRISS_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('triss_wp_list_categories') ){
	function triss_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'triss_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('triss_wp_list_archive') ){
	function triss_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'triss_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('triss_wpcf7_form_elements') ){
	function triss_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'triss_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Pagination for Blog and Portfolio
 * --------------------------------------------------------------------------- */
function triss_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'triss') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fas fa-angle-double-left"></i>'.esc_html__('Prev', 'triss'),
					'next_text'          => esc_html__('Next', 'triss').'<i class="fas fa-angle-double-right"></i>',
					'type'               => 'list'
				);
...
function triss_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'triss' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'triss'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'triss' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'triss' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'triss' );
		}
		...
		$title = esc_html__('Tag Archives','triss');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function triss_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function triss_wp_kses($content) {
 ...
 */
function triss_before_after_widget ( $content ) {
	$allowed_html = array(
		'aside' => array(
			'id'    => array(),
			'class' => array()
 ...
 */
function triss_widget_title( $content ) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('triss_hex2rgb')) {

	function triss_hex2rgb($hex) {
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function triss_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'triss');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'triss'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '  <div class="comment-body">';
					...
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'triss');
					endif;
					edit_comment_link(esc_html__('Edit', 'triss'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'triss'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
function triss_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function triss_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function triss_active_custom_widgetarea( $type, $id, $position = 'left' ) {
 ...
 * --------------------------------------------------------------------------- */
function triss_global_variables($variable = '') {
...
/* ---------------------------------------------------------------------------
 * Walker Page for triss_new_wp_page_menu
 * --------------------------------------------------------------------------- */
class TRISS_Walker_Page extends Walker_Page {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ($item->object == 'dt_mega_menus' && $depth == 1) {
			...
			if( !empty( $sc_custom_css ) ) {
				...
				wp_add_inline_style( 'triss-custom-inline', wp_strip_all_tags( $sc_custom_css ) );
			}

			$custom_css = get_post_meta($item->object_id, '_wpb_post_custom_css', true);
			if( !empty( $custom_css ) ) {
				wp_enqueue_style( 'triss-custom-inline' );
				wp_add_inline_style( 'triss-custom-inline', wp_strip_all_tags( $custom_css ) );
			}                
		} else {
 ...
 * --------------------------------------------------------------------------- */
add_filter('upload_mimes', 'triss_upload_mimes');
function triss_upload_mimes( $existing_mimes = array() ){
...
* Whitelist Associate
* --------------------------------------------------------------------------- */
if ( ! function_exists( 'triss_array_whitelist_assoc' ) ) {
	function triss_array_whitelist_assoc( Array $array1, Array $array2 ) {
		if ( func_num_args() > 2 ) {
			$args = func_get_args();
			array_shift( $args );
...
* Post Type Support
* --------------------------------------------------------------------------- */
add_filter( 'fw_ext_page_builder_supported_post_types', 'triss_limit_post_types_support', 1 );
if (!function_exists('triss_limit_post_types_support')) {

	function triss_limit_post_types_support( $all_post_types ) {
		$white_listed_post_types = array( '' ); //allowed custom post type names
		$post_types              = triss_array_whitelist_assoc( $all_post_types, $white_listed_post_types );
		return $post_types;
	}
}
