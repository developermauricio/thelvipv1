<?php 
class Triss_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			"classname"   => 'widget_flickr',
			'description' => esc_html__('A widget that show last flickr photo streams', 'triss')
		);

		parent::__construct( false, TRISS_THEME_NAME.esc_html__(' Flickr Widget','triss'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr( $this->get_field_id('flickr_id') ); ?>">
        		<?php esc_html_e('Flickr ID:','triss');?>
         ...
         <p>
         	<label for="<?php echo esc_attr($this->get_field_id('size'));?>">
         		...
         		<select class="widefat" id="<?php echo esc_attr($this->get_field_id('size')); ?>" name="<?php echo esc_attr($this->get_field_name('size')); ?>"><?php
         			$a = array(
						"s" => esc_html__("square",'triss'),
						"t" => esc_html__("thumbnail",'triss'),
						"m" => esc_html__("medium",'triss')
         			);
