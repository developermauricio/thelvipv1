...
if (! class_exists ( 'DTBaseSC' ) ) {
    ...
    class DTBaseSC {
        ...
        function dt_current_skin( $code = 'primary-color' ) {
            ...
            $color = '';
            $mode = get_theme_mod( 'use-predefined-skin', triss_defaults('use-predefined-skin') );

            if( $mode ) {
                $skin = get_theme_mod( 'predefined-skin', triss_defaults('predefined-skin' ) );
                $skin = triss_skins( $skin );
                $color = $skin[$code];
            } else {
                $color = get_theme_mod( $code, triss_defaults( $code ) );
            }
