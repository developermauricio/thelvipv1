...
if (! class_exists ( 'DTHeaderPostType' ) ) {
	...
	class DTHeaderPostType {
		...
		function __construct() {
			...
			add_filter( 'triss_vc_default_cpt', array( $this, 'dt_enable_vc' ) );
		...
		function dt_load_header_modules() {
			...
			$get_post_type = isset($_GET['post']) ? $_GET['post'] : '';

			if( 'post.php' === $pagenow ) {
				if( isset( $get_post_type ) && ( get_post_type( $get_post_type ) ==='dt_headers' ) ) {
					$this->dt_header_modules();
				}
			}

			if( ( 'post-new.php' === $pagenow ) && ( $get_post_type === 'dt_headers' ) ) {
				$this->dt_header_modules();
			}			
