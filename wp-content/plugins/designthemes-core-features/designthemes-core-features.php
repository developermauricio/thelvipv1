...
if (! class_exists ( 'DTCorePlugin' )) {
	...
	class DTCorePlugin {
		...
		function __construct() {
			...
			if( ($name == 'Triss') || ($name == 'Triss Child' ) || ($textdomain == 'triss' )) {
			} else {
				if( $template == 'triss' ) {
				} else {
					return;
			...
			define( 'TRISS_CORE_PLUGIN', plugin_dir_path ( __FILE__ ) );
		...
		function dt_widgets_init() {
			register_widget('Triss_Twitter');
			register_widget('Triss_Flickr');
			register_widget('Triss_Recent_Posts');
		}
