	...
	function dtportfolio_terms($attrs, $content = null) {
		...
		if($attrs['portfolio_id'] != '') {
			...
			if($attrs['terms'] == 'tags') {
				...
				if($attrs['with_icon']) {
					$output .= get_the_term_list($attrs['portfolio_id'], 'portfolio_tags', '<p class="dtportfolio-categories"><i class="fas fa-tags"></i>', ', ', '</p> ');
				} else {
					$output .= get_the_term_list($attrs['portfolio_id'], 'portfolio_tags', '<p class="dtportfolio-categories">', ', ', '</p>');
				}
			} else{
